...
class Player {
  ...
  update(input, camera, timeDelta, map) {
    if (input.leftright !== 0) {
      this.angle += input.leftright * 0.1 * timeDelta * 0.01;
    }
    if (input.forwardbackward !== 0) {
      const angle = camera.angle;
      this.x -= input.forwardbackward * Math.sin(angle) * timeDelta * 0.05;
      this.y -= input.forwardbackward * Math.cos(angle) * timeDelta * 0.05;
    }
  }
}
...
class Camera {
  ...
  update(input, timeDelta, map) {
    ...
    this.height = map.altitude[mapOffset] + 20;
  }
}
...
class ScreenData {
  ...
  drawPlayer(x, y) {
    ...
    let offset = ((y * this.canvas.width) + x) | 0;
    for (let k = y | 0; k < y + 2 | 0; k = k + 1 | 0) {
      this.buf32[offset | 0] = this.backgroundColor | 0;
      offset = offset + this.canvas.width | 0;
    }
  }
  ...
  render(camera, player, map) {
    ...
    for (let z = 1; z < camera.distance; z += deltaZ) {
      ...
      for (let i = 0; i < screenWidth; i++) {
        ...
        if (Math.abs(plx - player.x) < 2 && Math.abs(ply - player.y) < 2) {
          this.drawPlayer(i, heightOnScreen | 0);
        } else {
...
class Viewer {
  ...
  updateCamera() {
    ...
    this.player.update(this.input, this.camera, timeDelta, this.map);
