...
function init(view, canvasElementId) {
    ...
    view.canvasElement.width = view.canvasWidth;
    view.canvasElement.height = view.canvasHeight;
    view.context = view.canvasElement.getContext("2d");
    view.context.scale(scaleRatio, scaleRatio);
}
...
function gameTickElapsed(millisecondsProgressed) {
    ...
    if (tiltUpPressed) tiltUp();
    if (tiltDownPressed) tiltDown();
...
function renderPlayView() {
    ...
    //
    //     // var tgtx = camera.x - cosang * 2;
    //     // var tgty = camera.y - sinang * 2;
    //
    //     // plx += camera.x;
    //     // ply += camera.y;
    //
    ...
    while (z < camera.viewdistance) {
        ...
        // var pry = -sinang * z - cosang * z;
        //
        //     var dx = (prx - plx) / playView.canvasWidth;
        //     var dy = (pry - ply) / playView.canvasWidth;
    ...
    Promise.all([
        ...
        .then(function() {
            ...
            document.addEventListener('keyup', function(e) {
                ...
                if (event.keyCode === 82) tiltUpPressed = false;
                if (event.keyCode === 86) tiltDownPressed = false;
            });
        });
...
var tiltUpPressed = false;
var tiltDownPressed = false;
...
function tiltUp() {
    if (camera.horizon > camera.speeds.max.horizonMin) camera.horizon -= camera.speeds.tilt;
}

function tiltDown() {
    if (camera.horizon < camera.speeds.max.horizonMax) camera.horizon += camera.speeds.tilt;
}
// END CONTROL
